...
APP_NAME="nightshift"
NIGHTSHIFT_REPO="github.com/marcus/nightshift/cmd/nightshift@latest"
...
if go install "$NIGHTSHIFT_REPO" 2>&1; then
    # 4. Verify successful installation
    if command -v nightshift >/dev/null 2>&1 || [ -f "$GOPATH/bin/nightshift" ]; then
...
echo "You may install manually later: go install $NIGHTSHIFT_REPO"
exit 0
