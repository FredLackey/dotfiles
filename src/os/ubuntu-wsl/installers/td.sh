...
APP_NAME="td"
TD_REPO="github.com/marcus/td@latest"
...
if go install "$TD_REPO" 2>&1; then
    # 4. Verify successful installation
    if command -v td >/dev/null 2>&1 || [ -f "$GOPATH/bin/td" ]; then
...
echo "You may install manually later: go install $TD_REPO"
exit 0
