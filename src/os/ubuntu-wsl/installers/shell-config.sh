...
TMUX_CONF_CURRENT=false
STARSHIP_CURRENT=false
...
if check_symlink "$FILES_DIR/.tmux.conf" "$HOME/.tmux.conf"; then
    TMUX_CONF_CURRENT=true
fi

STARSHIP_DEST="$HOME/.config/starship.toml"
if check_symlink "$FILES_DIR/starship.toml" "$STARSHIP_DEST"; then
    STARSHIP_CURRENT=true
fi

if [ "$BASHRC_CURRENT" = true ] && [ "$BASH_PROFILE_CURRENT" = true ] && [ "$TMUX_CONF_CURRENT" = true ] && [ "$STARSHIP_CURRENT" = true ]; then
...
# Link .tmux.conf
backup_and_link "$FILES_DIR/.tmux.conf" "$HOME/.tmux.conf"

# Link starship.toml to ~/.config/starship.toml
if [ -f "$FILES_DIR/starship.toml" ]; then
    mkdir -p "$HOME/.config"
    backup_and_link "$FILES_DIR/starship.toml" "$STARSHIP_DEST"
fi
