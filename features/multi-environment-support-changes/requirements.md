...
- Create a new folder: `src/os/`.
- Within `src/os/`, create a dedicated subdirectory for each target environment (e.g., `macos`, `ubuntu-wsl`, etc.).

### 2. Relocation of Environment Scripts
Move the specific environment setup scripts from `src/` into their respective folders within `src/os/` and rename them to a standard entry point (e.g., `setup.sh` or `setup.ps1`).

**Proposed Mapping:**
- `src/setup_macos.sh` &rarr; `src/os/macos/setup.sh`
- `src/setup_ubuntu-wsl.sh` &rarr; `src/os/ubuntu-wsl/setup.sh`
- `src/setup_ubuntu-desktop.sh` &rarr; `src/os/ubuntu-desktop/setup.sh`
- `src/setup_ubuntu-server.sh` &rarr; `src/os/ubuntu-server/setup.sh`
- `src/setup_windows.ps1` &rarr; `src/os/windows/setup.ps1`
...
- Based on the identification, they must invoke the appropriate child script located in `src/os/<environment>/setup.*`.
